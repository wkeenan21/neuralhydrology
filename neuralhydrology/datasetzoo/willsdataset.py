...
def load_timeseries(data_dir: str, basin: str) -> pd.DataFrame:
    ...
    basin_file = r"{}\{}.csv".format(data_dir, basin)
...
def load_cc(data_dir: str) -> pd.DataFrame:
    ...
    # Convert gage to str and add leading zeros if the gauge id is shorter than 8 characters.
    df["gage"] = df["gage"].apply(lambda x: str(x).zfill(8))
    df.set_index("gage", inplace=True)
